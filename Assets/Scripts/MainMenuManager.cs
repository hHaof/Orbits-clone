...
public class MainMenuManager : MonoBehaviour
{
    ...
    IEnumerator ShowScore()
    {
        ...
        int _currentScore = GameManager.Instance.CurrentScore;
        int _highScore = GameManager.Instance.HighScore;

        if (_currentScore > _highScore)
        {
            _newBestText.gameObject.SetActive(true);
            GameManager.Instance.HighScore = _currentScore;
        }
        else
        {
            _newBestText.gameObject.SetActive(false);
        ...
        while(timeElapsed < 1f){
            ...
            tempScore = (int)(_speedCurve.Evaluate(timeElapsed) * _currentScore);
            _scoreText.text = tempScore.ToString();
            yield return null;
        }

        tempScore = _currentScore;
        _scoreText.text = tempScore.ToString();
